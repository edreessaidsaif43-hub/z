...
export default async function handler(req, res) {
  ...
  const created = await createPortfolio(payload, { forceNew: true });
  ...
  if (created.error) {
    res.status(502).json({
      error: created.error,
      message: created.message || "Failed to refresh portfolio share link in Neon database backend.",
    });
    return;
  }
  ...
  res.status(201).json({ id: created.id, url, refreshed: true });
}
